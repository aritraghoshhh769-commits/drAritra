...
class InfiniteGridMenu {
  ...
  discLocations!: { [key: string]: WebGLUniformLocation | number };
  ...
  #init(onInit: ((sketch: InfiniteGridMenu) => void) | null) {
    ...
    const modelPosBuffer = makeBuffer(gl, this.discBuffers.vertices, gl.STATIC_DRAW);
    const modelUvBuffer = makeBuffer(gl, this.discBuffers.uvs, gl.STATIC_DRAW);
    if (!modelPosBuffer || !modelUvBuffer) return;

    this.discVAO = makeVertexArray(
      gl,
      [
        [modelPosBuffer, this.discLocations.aModelPosition as number, 3],
        [modelUvBuffer, this.discLocations.aModelUvs as number, 2]
      ],
  ...
  #initDiscInstances(count: number) {
    ...
    for (let i = 0; i < count; ++i) {
      const instanceMatrixArray = mat4.create();
  ...
  #animate(deltaTime: number) {
    ...
    positions.forEach((p, ndx) => {
      ...
      this.discInstances.matricesArray.set(matrix, ndx * 16);
    });
  ...
  #onControlUpdate(deltaTime: number) {
    ...
    this.onStateUpdate({
      faces: facesState,
      activeItemIndex: this.nearestVertexIndex,
      isMoving: this.movementActive
    });
  }
