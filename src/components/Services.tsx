...
const ServiceCard = ({ service, onCtaClick }: { service: Service; onCtaClick: () => void }) => (
  ...
  >
    ...
    <p className="text-xs text-[#6B7280] leading-relaxed mb-3 md:text-sm md:mb-5">{service.description}</p>

    <div className="mt-auto">
      <Button
        className="bg-white text-[#1F2A37] rounded-full px-3 h-auto py-1.5 text-xs font-medium shadow-sm hover:bg-gray-100 md:px-6 md:py-2.5 md:text-sm"
        onClick={onCtaClick}
      >
        {service.ctaText}
      </Button>
