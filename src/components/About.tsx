  ...
  return (
    ...
    >
      <div className="container mx-auto max-w-5xl md:max-w-7xl px-4">
        <motion.div 
          className="bg-white/90 backdrop-blur-md rounded-2xl lg:rounded-[24px] shadow-[0_20px_40px_rgba(0,0,0,0.06)] border border-white/60 flex flex-col lg:flex-row overflow-hidden"
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          <div className="w-full lg:w-7/12 flex flex-col justify-center items-start text-left space-y-4 p-6 md:space-y-5 md:p-8 lg:p-12">
            <motion.h2 
              id="about-dr-aritra"
              variants={itemVariants}
              className="text-2xl md:text-3xl font-bold tracking-tight md:tracking-[-0.5px]" 
              style={{ color: '#2f9aa0' }}
            >
            ...
            >
              {badges.map((badge) => (
                ...
                >
                  <badge.icon className="h-4 w-4 md:h-5 md:w-5 mr-2 shrink-0" style={{ color: '#2f9aa0' }} />
                  <span className="font-semibold text-xs md:text-sm leading-normal" style={{ color: '#2f9aa0' }}>{badge.text}</span>
                </motion.div>
              ))}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="pt-2 md:pt-3">
               <Button 
                 asChild 
                 className="w-full md:w-auto h-10 md:h-12 text-sm md:text-base font-semibold text-white rounded-lg md:rounded-xl shadow-[0_8px_20px_rgba(47,154,160,0.3)] md:shadow-[0_12px_30px_rgba(47,154,160,0.35)] transition-all duration-300 hover:scale-[1.03] hover:shadow-xl px-6 md:px-8"
                 style={{
